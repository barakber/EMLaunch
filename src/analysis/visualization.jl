...
function plot_monte_carlo_results(results; size=(1200, 900), body=EARTH)
    ...
    if has_atmosphere(body)
        Plots.hline!(p1, [100.0], label="Karman line (100 km)", linestyle=:dash, color=:red, linewidth=2)
        Plots.hline!(p1, [ustrip(u"km", h_LEO)], label="LEO ($(ustrip(u"km", h_LEO)) km)", linestyle=:dash, color=:purple, linewidth=2)
    end
...
function print_summary_statistics(results; body=EARTH)
    ...
    if has_atmosphere(body)
        if altitude > 400000
            println("\n  SUCCESS! Reached LEO altitude (400 km)")
            println("  Altitude above LEO: $(round((altitude-400000)/1000, digits=2)) km")
        elseif altitude > 100000
            println("\n  Crossed Karman line but below LEO")
            println("  Altitude above Karman line: $(round((altitude-100000)/1000, digits=2)) km")
            println("  Altitude deficit to LEO: $(round((400000-altitude)/1000, digits=2)) km")
        else
